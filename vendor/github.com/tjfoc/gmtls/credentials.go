...
func NewTLS(c *Config) credentials.TransportCredentials {
...
func NewClientTLSFromCert(cp *sm2.CertPool, serverNameOverride string) credentials.TransportCredentials {
	return NewTLS(&Config{ServerName: serverNameOverride, RootCAs: cp})
}
...
func NewClientTLSFromFile(certFile, serverNameOverride string) (credentials.TransportCredentials, error) {
	...
	return NewTLS(&Config{ServerName: serverNameOverride, RootCAs: cp}), nil
}

// NewServerTLSFromCert constructs TLS credentials from the input certificate for server.
func NewServerTLSFromCert(cert *Certificate) credentials.TransportCredentials {
	return NewTLS(&Config{Certificates: []Certificate{*cert}})
}
...
func NewServerTLSFromFile(certFile, keyFile string) (credentials.TransportCredentials, error) {
	...
	return NewTLS(&Config{Certificates: []Certificate{cert}}), nil
}
