...
/**
 * @description: 服务服务端的混合握手，根据客户端协议类型自动选择GMSSL协议或TLS系列协议
 * @author: Andycao
 * @date: 2023/6/25
 * @params
 * @return:
...
/**
 * @description: 处理 TLS 客户端 Hello消息 Code segment copy from: gmtls/handshake_server.go:122
 * @author: Andycao
 * @date: 2023/6/21
 * @params c: 连接对象,hs: TLS服务端握手状态
 * @return: 是否重用会话，true/false 重用/不重用,错误信息
 */
...
Curves:
	...
	if priKey, ok := hs.cert.PrivateKey.(crypto.Signer); ok {
		switch priKey.Public().(type) {
		...
		default:
			_ = c.sendAlert(alertInternalError)
			return false, fmt.Errorf("tls: unsupported signing key type (%T)", priKey.Public())
		}
	}
	if priKey, ok := hs.cert.PrivateKey.(crypto.Decrypter); ok {
		switch priKey.Public().(type) {
		case *rsa.PublicKey:
			hs.rsaDecryptOk = true
		default:
			_ = c.sendAlert(alertInternalError)
			return false, fmt.Errorf("tls: unsupported decryption key type (%T)", priKey.Public())
		}
	}
