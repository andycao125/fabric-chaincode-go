...
func (hs *clientHandshakeStateGM) doFullHandshake() error {
	...
	if c.handshakes == 0 {
		...
		for i, asn1Data := range certMsg.certificates {
			...
			switch i {
			case 0:
				if cert.KeyUsage == 0 || (cert.KeyUsage&(sm2.KeyUsageDigitalSignature|cert.KeyUsage&sm2.KeyUsageContentCommitment)) == 0 {
					c.sendAlert(alertInsufficientSecurity)
			...
			case 1:
				if cert.KeyUsage == 0 || (cert.KeyUsage&(sm2.KeyUsageDataEncipherment|sm2.KeyUsageKeyEncipherment|sm2.KeyUsageKeyAgreement)) == 0 {
					c.sendAlert(alertInsufficientSecurity)
...
findCert:
	for i, chain := range c.config.Certificates {
		...
		for j, cert := range chain.Certificate {
			...
			if x509Cert.PublicKeyAlgorithm == sm2.ECDSA {
