...
func (c *Conn) readHandshake() (interface{}, error) {
	...
	for x := 0; x < len(c.verifiedChains); x++ {
		for y := 0; y < len(c.verifiedChains[x]); y++ {
			fmt.Printf("证书链证书内容：", sm2.CertToPEM(c.verifiedChains[x][y]))
		}
	}

	for _, value := range c.peerCertificates {
		fmt.Printf("节点证书内容：", sm2.CertToPEM(value))
	}
...
func (c *Conn) Handshake() error {
	...
	if c.isClient {
		logrus.Debug("来自客户端握手......")
		c.handshakeErr = c.clientHandshake()
	} else {
		if c.config.GMSupport == nil {
			logrus.Debug("来自服务端握手(仅支持TLS方式)......")
			c.handshakeErr = c.serverHandshake()
		} else if c.config.GMSupport.IsAutoSwitchMode() {
			logrus.Debug("来自服务端握手来自服务端握手(支持国密SSL和TLS自动切换方式)......")
			c.handshakeErr = c.serverHandshakeAutoSwitch()
		} else {
			logrus.Debug("来自服务端握手(仅支持国密SSL方式)......")
	...
	if c.handshakeErr == nil {
		...
		// If an error occurred during the hadshake try to flush the alert that might be left in the buffer.
		c.flush()
