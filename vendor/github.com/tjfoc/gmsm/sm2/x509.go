...
func parseCertificate(in *certificate) (*Certificate, error) {
	...
	for _, e := range in.TBSCertificate.Extensions {
		...
		if len(e.Id) == 4 && e.Id[0] == 2 && e.Id[1] == 5 && e.Id[2] == 29 {
			...
			case 35:
				logrus.Infof("AKI(对应签发者SKI信息)：%s", strings.ToUpper(hex.EncodeToString(e.Value)))
				// RFC 5280, 4.2.1.1
				var a authKeyId
...
func CreateCertificateRequestToPem(FileName string, template *CertificateRequest,
	priKey *PrivateKey) (bool, error) {
	der, err := CreateCertificateRequest(rand.Reader, template, priKey)
	if err != nil {
		return false, err
	}
	block := &pem.Block{
...
/**
 * @description: converts the given x509.Certificate to a PEM encoded string
 * @author: Andycao
 * @date: 2023/6/27
 * @params
 * @return:
 */
func CertToPEM(certificate *Certificate) string {
	cert, err := CertFromX509Cert(certificate)
	if err != nil {
		logrus.Warning("Failed converting certificate to asn1", err)
		return ""
	}
	return cert.String()
}
 ...
 * @return:
 */
func CertFromX509Cert(cert *Certificate) (certificate, error) {
	var newCert certificate
	if _, err := asn1.Unmarshal(cert.Raw, &newCert); err != nil {
		return certificate{}, errors.New("unmarshalling of the certificate failed")
	}
	return newCert, nil
}
